...
import { cascadeDelay, keyForSwipe, photoForArrow } from "../src/app/misc/navigation.mjs";
...
assert.equal(cascadeDelay(0, 43), 0);
assert.equal(cascadeDelay(42, 43), 240, "Large galleries finish staggering after 20 slots");
assert.equal(cascadeDelay(4, 5), 48, "Small galleries give each photo its own slot");
